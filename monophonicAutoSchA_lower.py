...
"""
Prioritizes second-most "structural" line of notes
Example terminal call:

uv run monophonicAutoSchA.py \
  --json outputs/inference/Bach_880.json \
  --output-xml outputs/reductions/Bach-monophonic.musicxml \
  --layer 2
...
def assign_slices_to_utility_streams(slices):
    """
    Stream 1 = second-highest utility selected tone in the slice
               (fallback to highest if only one tone exists)
    Stream 2 = highest utility selected tone in the slice

    This lets the monophonic reducer treat the second-ranked structural
    tone as the melody-like line.
    """
    stream_cells = {
        1: defaultdict(list),  # melody-like stream
        2: defaultdict(list),  # higher-utility support stream
    }
    ...
    for sl in slices:
        ...
        if len(sl.tones) >= 2:
            primary_pitch = (sl.tones[1].pitch_midi,)   # 2nd highest utility
            secondary_pitch = (sl.tones[0].pitch_midi,) # highest utility
        elif len(sl.tones) == 1:
            primary_pitch = (sl.tones[0].pitch_midi,)   # fallback
            secondary_pitch = ()

        stream_cells[1][sl.measure_index].append(
            MeasureCell(
                start_local=sl.start_local,
                end_local=sl.end_local,
                pitches=primary_pitch,
            )
        )

        stream_cells[2][sl.measure_index].append(
            MeasureCell(
                start_local=sl.start_local,
                end_local=sl.end_local,
                pitches=secondary_pitch,
            )
        )
